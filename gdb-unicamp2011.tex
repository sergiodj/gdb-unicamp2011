...
\begin{frame}[fragile]
	\frametitle{O papel da Red Hat nessa história}
	\begin{itemize}
		\item{GDB, GCC, glibc, Kernel Linux, virtualização...}
		\item{Participa ativamente do projeto (2 mantenedores globais no GDB).}
		\begin{itemize}
			\item{Suporte a extensões em Python}
			\item{SystemTap probes em userspace}
			\item{Melhorias no desempenho (utilizando probes)}
			\item{Várias melhorias no suporte a C++ (Projeto Archer)}
			\item{...}
		\end{itemize}
	\end{itemize}
\end{frame}
